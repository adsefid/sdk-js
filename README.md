...
- `TemplateParameterType` — `"string" | "number"` (the documented complete public set)
...
make lint   # npm run lint    (biome check .) + tsc --noEmit
make build  # npm run build   (tsup — emits ESM, CJS, and .d.ts into dist/)
```

Runnable usage samples live in [`examples/`](./examples) (not published to npm — see `"files"` in `package.json`). Run one directly with `tsx`:
